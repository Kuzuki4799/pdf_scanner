package com.vmb.picker.videoorphoto.tools;
...
import com.vmb.picker.videoorphoto.listener.OnCallbackListener;
    ...
    public static void setOrientationAsynchronous(Context context, LocalMedia media,
        ...
        PictureThreadUtils.executeByIo(new PictureThreadUtils.SimpleTask<Integer>() {
            ...
            public Integer doInBackground() {
                int orientation = 0;
                if (PictureMimeType.isHasImage(media.getMimeType())) {
                    orientation = com.vmb.picker.videoorphoto.tools.MediaUtils.getImageOrientationForUrl(context, media.getPath());
                } else if (PictureMimeType.isHasVideo(media.getMimeType())) {
                    if (PictureMimeType.isContent(media.getPath())) {
                        orientation = com.vmb.picker.videoorphoto.tools.MediaUtils.getVideoOrientationForUri(context, Uri.parse(media.getPath()));
                    } else {
                        orientation = com.vmb.picker.videoorphoto.tools.MediaUtils.getVideoOrientationForUrl(media.getPath());
                    }
                }
                return orientation;
            }

...
import com.vmb.picker.videoorphoto.PictureSelectionModel;
import com.vmb.picker.videoorphoto.PictureSelector;
import com.vmb.picker.videoorphoto.R;
import com.vmb.picker.videoorphoto.config.InstagramSelectionConfig;
import com.vmb.picker.videoorphoto.config.PictureConfig;
...
import com.vmb.picker.videoorphoto.engine.CacheResourcesEngine;
import com.vmb.picker.videoorphoto.engine.ImageEngine;
import com.vmb.picker.videoorphoto.entity.LocalMedia;
import com.vmb.picker.videoorphoto.glide.GlideCacheEngine;
import com.vmb.picker.videoorphoto.glide.GlideEngine;
import com.vmb.picker.videoorphoto.listener.OnResultCallbackListener;
import com.vmb.picker.videoorphoto.style.PictureCropParameterStyle;
import com.vmb.picker.videoorphoto.style.PictureParameterStyle;
import com.vmb.picker.videoorphoto.style.PictureWindowAnimationStyle;
...
public final class VMBGallery {
    ...
    public static PictureSelectionConfig appVMBOfVideo(Activity activity) {
        return appVMBOfVideo(activity, "", false);
    }

    public static PictureSelectionConfig appVMBOfVideo(Activity activity, String originalPath, boolean isShowIsCheck) {
        PictureSelectionModel pictureSelectionModel = appVMBOfVideo(activity.getApplicationContext(), PictureSelector.create(activity).openGallery(PictureMimeType.ofVideo()));
        pictureSelectionModel.imageEngine(GlideEngine.createGlideEngine());// 外部传入图片加载引擎，必传项
        pictureSelectionModel.loadCacheResourcesCallback(GlideCacheEngine.createCacheEngine());// 获取图片资源缓存，主要是解决华为10部分机型在拷贝文件过多时会出现卡的问题，这里可以判断只在会出现一直转圈问题机型上使用
        pictureSelectionModel.isShowIsCheck(isShowIsCheck);
        if (!TextUtils.isEmpty(originalPath))
            pictureSelectionModel.setOriginalPath(originalPath);
        return pictureSelectionModel.selectionConfig;
    }

    public static PictureSelectionModel appVMBOfVideo(Context context, PictureSelectionModel selectionModel) {
        return applyInstagramOptions(context, InstagramSelectionConfig.createConfig(), selectionModel);
    }
    ...
    public static PictureSelectionModel applyInstagramOptions(Context context, InstagramSelectionConfig instagramConfig, PictureSelectionModel selectionModel) {
        return selectionModel
                ...
                .maxVideoSelectNum(100) // 视频最大选择数量，如果没有单独设置的需求则可以不设置，同用maxSelectNum字段
...
//        // 自定义相册右侧文本内容设置
    ...
    public static void setCurrentTheme(int currentTheme) {
        VMBGallery.currentTheme = currentTheme;
    }
}
